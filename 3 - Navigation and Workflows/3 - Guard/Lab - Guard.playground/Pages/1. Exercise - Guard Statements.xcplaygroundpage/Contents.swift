...
func add(x: Int?, y: Int?) -> Int? {
    guard let xnum = x else {return nil}
    guard let ynum = y else {return nil}
...
if let person = users {
    print("user name: \(person.firstName) \(person.lastName), age: \(person.age).")
}
